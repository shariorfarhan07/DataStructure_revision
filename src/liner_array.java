...
public class liner_array {
        ...
        public static void printReverse(int [] source){
            System.out.print('{');
            for (int i = 0; i < source.length; i++) {
                System.out.print(source[source.length-1-i]);
                if (source.length-1!=i)
                    System.out.print(',');
                else
                    System.out.println('}');
        ...
        public static int [] rotateLeft(int [] source, int k){
            ...
            int[] newArray=shiftLeft(source,k);
            for (int i = 0; i< extra.length; i++) {
                newArray[source.length-k+i]=extra[i];
                }
            return newArray; // remove this line
        }
        ...
        public static int [] rotateRight(int [] source, int k){
            int [] leftelement=new int[k];
            int itarator=0;
            for (int i = 0; i < k; i++) {
                leftelement[i]=source[source.length-k+i];
            }
            source=shiftRight(source,k);
            for (int i = 0; i < k; i++) {
                source[i]=leftelement[i];
            }
        ...
        public static boolean removeAll(int [] arr, int size, int elem){
            ...
            for (int i = 0; i < size; i++) {
                if (arr[i]==elem){
                    b=true;
                    int ind=i;
                    while (size!=ind && ind+1< arr.length){
                        arr[ind]=arr[ind+1];
                        ind++;
//                        System.out.println(Arrays.toString(arr)+ind);
                    }
                    size--;
                }
            }
